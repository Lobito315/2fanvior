...
export default function LiveStreamView({ creatorHandle, creatorName, creatorId }: LiveStreamViewProps) {
  const { data: session } = useSession();
  const [token, setToken] = useState<string | null>(null);
  const [room, setRoom] = useState<string>(`room-${creatorId}`);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    // Simulate local addition for now
    setLiveMessages(prev => [...prev, {
        ...
        <section className="flex-1 p-6 flex flex-col">
          <div className="w-full flex-1 bg-surface-container-highest rounded-2xl border border-outline-variant/20 overflow-hidden relative shadow-2xl flex flex-col group">
            ...
            {token ? (
              ...
              <div className="absolute inset-0 bg-surface-container-lowest flex flex-col items-center justify-center">
                 ...
                 <p className="text-outline text-sm mt-2 font-body">Connecting to {creatorName}'s broadcast node.</p>
              </div>
            )}
          ...
          <div className="mt-6 flex justify-between items-center">
            <div className="flex flex-col">
              <h1 className="font-headline text-2xl font-bold tracking-tight uppercase">{creatorName}'s Channel</h1>
              <p className="text-sm text-outline mt-1 font-label uppercase tracking-widest flex items-center gap-2">
                <span className="material-symbols-outlined text-sm">public</span>
                Live from @{creatorHandle}
              </p>
            </div>
